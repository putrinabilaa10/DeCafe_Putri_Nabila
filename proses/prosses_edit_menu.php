    ...
    if ($statusUpload == 0) {
      ...
      if (mysqli_num_rows($select) > 0) {
        ...
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
          $query = mysqli_query($conn, "UPDATE tb_daftar_menu SET Gambar='".$kode_rand. $_FILES['foto']['name'] ."' , Nama_menu = '$nama_menu' , Keterangan ='$keterangan', Kategori='$kat_menu',Harga ='$harga', Stok='$stok' WHERE id='$id'");
          if ($query) {
